print('Training early fusion (ViT)...')
import os 
...
class EarlyFusionModel(torch.nn.Module):
    ...
    def forward(
        ...
        # early fusion - cnn with vit output + bert output as input
...
# train_loop
def train_loop(model, model_type, train_dataloader, val_dataloader, device, num_epochs, model_save_path=Path('./vit-models-early-fusion')):
...
for idx in ablation_study_attn_blocks:
    model_save_path = Path(f'./models-early-fusion_{str(idx)}-attn-blocks-vit')
