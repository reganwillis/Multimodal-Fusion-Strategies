...
class MidFusionModel(torch.nn.Module):
    """
    Intermediate fusion of MobileNetV2 and BERT.
    """
    def __init__(self, fuse_place):
        super().__init__()
        self.num_labels = 2
        ...
        self.fusion_layers.append(torch.nn.Linear(1536, channels[1]))  # 3
        self.fusion_layers.append(torch.nn.Linear(1536, channels[1]))  # 6
    ...
    def forward(
        self,
        pixel_values: Optional[torch.Tensor] = None,  # vit val
        ...
        hidden_states_vit = vit_embedding_output
        ...
        truncate_layers = idx[3]
        x = None
        fused_outputs = []
        j = 0
        for i in range(len(self.bert.encoder.layer)-truncate_layers):
            #print('LAYER', i+1)
            hidden_states_vit = self.vit.encoder.layer[i](hidden_states_vit)
...
# train_loop
def train_loop(model, model_type, train_dataloader, val_dataloader, device, num_epochs, model_save_path=Path('./vit-fused-mid-fusion')):
...
# first 3 indicies are the place to freeze from
# last index is the amount of layers to truncate
fuse_place = [[8,6,4,4]]

for idx in fuse_place:
    tmp = [str(el) for el in idx]
    tmp = '-'.join(tmp)
    model_save_path = Path(f'./models-mid-fusion_{tmp}-fuse-vit')

    print('...creating mid fusion model')
    early_fusion_model = MidFusionModel(idx)
    early_fusion_model.to(DEVICE)

    # run training
    print('training mid fusion model..')
    ...
    # run evaluation (test)
    early_fusion_model = MidFusionModel(idx)
    saved_model_path = model_save_path / 'late_fusion_state_dict.pt'
    early_fusion_model.load_state_dict(torch.load(saved_model_path))
    exit
    early_fusion_model.to(DEVICE)

    test_loss, test_accuracy = evaluate(early_fusion_model, 'late_fusion', base_test_dataloader, DEVICE, len(base_test_dataloader))
    print(
        f'TEST (mid fusion)--> DONE, ' +
