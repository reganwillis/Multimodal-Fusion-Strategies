...
class MidFusionModel(torch.nn.Module):
    ...
    def __init__(self, fuse_place):
        ...
        self.fusion_layers.append(torch.nn.Linear(832, channels[1]))  # 3
        ...
        self.fusion_layers.append(torch.nn.Linear(832, channels[1]))  # 6
    ...
    def forward(
        ...
        for i in range(len(self.bert.encoder.layer)-truncate_layers):
            ...
            if i == self.fuse_place[0]-1 or i == self.fuse_place[1]-1 or i == self.fuse_place[2]-1:
                flat1 = self.conv_layers[j](hidden_states_mobilenet).squeeze(-1).squeeze(-1)
...
# train_loop
def train_loop(model, model_type, train_dataloader, val_dataloader, device, num_epochs, model_save_path=Path('./models-mid-fusion')):
...
for idx in fuse_place:
    ...
    tmp = '-'.join(tmp)
    model_save_path = Path(f'./models-mid-fusion_{tmp}-fuse')
